...
(function($){
	...
	function createBody(target,records,params){
		...
		$(".sea_grid_1_body table tr:even,.sea_grid_2_body table tr:even",target).addClass("sea_grid_tr sea_grid_even") ;
		$(".sea_grid_1_body table tr:odd,.sea_grid_2_body table tr:odd",target).addClass("sea_grid_tr sea_grid_odd") ;
	...
	function loadMask(target , isShow){
		...
		mask.show() ;
		maskMsg.html(target.data("options").loadMsg).css({
					"left":"45%",
					"top":"45%"
		}).show();
	}
...
//		$(target).delegate(".sea_grid_sort_col",'click',function(){
...
//		}) ;
	...
	function setTitle(target,title){
		title&&target.find(".sea_grid_title span").html(title) ;
		!title&&target.find(".sea_grid_title").hide() ;
	}
	...
	$.fn.Pager = function(settings){//pageSizes,curPage,totalRecord,limit,selectPage
		...
		function event(){
			...
			me.find(".sea_grid_pager_btn:not(.sea_state_disabled)").hover(function(){
