	...
	$.fn.tip = function(options) {
		var defaults = { 
	 	...
	 	if($("#tip_holder").length <= 0){
	 		var tip_holder = $('<div id="tip_holder" style="max-width:'+ opts.maxWidth +';"></div>');
			var tip_content = $('<div id="tip_content"></div>');
			var tip_arrow = $('<div id="tip_arrow"></div>');
			$("body").append(tip_holder.html(tip_content).prepend(tip_arrow.html('<div id="tip_arrow_inner"></div>')));
		} else {
			var tip_holder = $("#tip_holder");
			tip_holder.mouseover(function(){
				tip_holder.hide();
			});;
			...
			if(org_title != ""){
				...
				function active_tip(){
					...
					tip_arrow.css({"margin-left": arrow_left+"px", "margin-top": arrow_top+"px"});
					tip_holder.css({"margin-left": marg_left+"px", "margin-top": marg_top+"px"}).attr("class","tip"+t_class);
					
					if (timeout){ clearTimeout(timeout); }
					timeout = setTimeout(function(){ tip_holder.stop(true,true).fadeIn(opts.fadeIn); }, opts.delay);	
				}
