define(function(require, exports, module) {
	...
	; (function($, window, document, undefined) {
		...
		Zmenu.prototype.variables = function(ele) {
			...
			this.mobileIcon = this.mobileNav.children('.sea_nav_icon');
			this.counter = 0;
		...
		Zmenu.prototype.setStyles = function(ele) {
			...
			this.links.css({
				...
				$(this).parent().addClass("sea_nav_active").siblings().removeClass("sea_nav_active").css('background-color', 'transparent');
			});
			if (this.active) {
		...
		Zmenu.prototype.createMobile = function(ele) {
			...
			this.allLinks.each(function(indx, element) {
				...
				txt = $(element).text(),
				hclass = $(element).parents('.sea_nav_sub').length,
				tr = '';
				if (hclass > 0) {
					for (i = 0; i < hclass; i++) {
						tr += '-'
		...
		Zmenu.prototype.change = function(ele) {
			...
			function(event) {
				if (event.type === 'mouseenter') {
					...
					if ($(this).hasClass('sea_nav_active') === false) {
						$(this).css('background-color', 'transparent').children('a').css('color', ele.op.fontColor)
					}
				}
			});
			if (this.op.transform ===true) {
				$(window).on('scroll.sea_navScrolling',
				function() {
